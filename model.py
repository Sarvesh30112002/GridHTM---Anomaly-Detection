...
"""
This module contains the implementation of the anomaly detection model using GRIDHTM.
"""
...
def grid_mean_aggr_func(anoms, axis=None):
    """
    Calculates the mean of the input array along the specified axis, ignoring any zeros.

    Args:
        anoms (numpy.ndarray): The input array.
        axis (int, optional): The axis along which to calculate the mean. Defaults to None.

    Returns:
        numpy.ndarray: The mean of the input array along the specified axis, with zeros replaced by NaNs.
    """
...
class SpatialPoolerArgs:
    """
    This class contains the arguments for configuring the spatial pooler algorithm.
    """
    def __init__(self):
...
class TemporalMemoryArgs:
    """
    A class to hold the arguments for the TemporalMemory class.
    """
    def __init__(self):
        self.columnDimensions = (0,)
        self.seed = 0
...
class SpatialPooler:
    """
    A class that implements the spatial pooling algorithm for a Hierarchical Temporal Memory (HTM) network.

    Args:
        sp_args (SpatialPoolerArgs): An object containing the parameters for the spatial pooler algorithm.

    Attributes:
        sp (algos.SpatialPooler): An instance of the spatial pooler algorithm.
        num_active_columns (int): The number of active columns in the spatial pooler.
    """
...
class GridHTM:
    """
    A class that implements a grid of Hierarchical Temporal Memory (HTM) networks for video processing.

    Args:
        frame_shape (Tuple[int, int]): The shape of the input video frames.
        sp_grid_size (Tuple[int, int]): The size of the grid of spatial poolers.
        tm_grid_size (Tuple[int, int]): The size of the grid of temporal memories.
        sp_args (SpatialPoolerArgs): An object containing the parameters for the spatial pooler algorithm.

    Attributes:
        spatial_poolers (List[List[SpatialPooler]]): A 2D list of spatial poolers.
        temporal_memories (List[List[TemporalMemory]]): A 2D list of temporal memories.
    """
